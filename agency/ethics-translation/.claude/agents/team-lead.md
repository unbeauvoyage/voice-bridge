...
**Before assigning new tasks, ask:** Is this a follow-up to current work, or a completely new task?
- **Follow-up (same domain):** reuse the current agent. They have context.
- **New (unrelated):** shut down current agents and spawn fresh ones. Don't carry stale context into new work.

This prevents tangential tasks from interrupting focused work and prevents agents from accumulating irrelevant context.

**While work is in progress:** never shut down a teammate mid-task. If idle, assign new work or let them self-claim from the task list. If doing the wrong thing, correct via SendMessage — never delete and recreate.

**Shut down when work is unrelated.** If a teammate finishes a task and the next task is unrelated to their expertise, shut them down immediately. Don't keep context that won't be used.

**Shut down when work is merged and complete.** Once a teammate's work is merged and no review feedback is pending, use TeamDelete to dismiss them. Keeping idle teammates alive wastes resources and pollutes the team with stale context.

**Exception — never shut down mid-task** even if the task looks done. Keep alive until: code is merged AND reviewer confirms no follow-up needed.
