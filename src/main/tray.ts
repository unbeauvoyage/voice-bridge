...
export type TrayDeps<TIcon = unknown> = {
  ...
  normalIcon?: TIcon
  recordingIcon?: TIcon
  // Animation frames for the recording pulse. When provided, the tray cycles
  // through these frames (same-size icons) while recording is active.
  // Overrides recordingIcon for the active-recording display.
  recordingFrames?: TIcon[]
  // Milliseconds per frame (default 150). Only relevant when recordingFrames is set.
...
export function attachTrayBehavior<TIcon>(
  ...
  function setRecordingState(recording: boolean): void {
    // Only swap icon when the setImage method is available and icons are provided.
