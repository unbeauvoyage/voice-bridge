...
- [ ] **[relay]** 8 pre-existing test failures on main — root cause: `src/__tests__/relay.test.ts` targets `http://localhost:8767` (production relay port) with no subprocess setup. Tests assume the prod relay is running and fail when prod is stale or running a divergent code version. This is the test-fixture-hitting-prod-port anti-pattern from WORKFLOW-REVIEW.md (documented in relay-auth.test.ts incident 2026-04-15). Failures: "ECONNREFUSED self-heal > sending to agent with dead port queues message", "store-and-forward: offline agent queuing" (×3), "voice message type accepted", "broadcastDashboard on queued message", "JSONL resilience malformed line", "relay_reply to CEO id consistency". NOT genuine regressions — confirmed by inspection: code paths exist in relay-lean.ts, but dist/ running as PID 15912 started at 00:19:41 (before 4628f19/a21f757). **Fix spec:** migrate `src/__tests__/relay.test.ts` (and `src/__tests__/zombie-channel-detection.test.ts`) to the subprocess pattern used by `relay-auth.test.ts`, `relay-hardening.test.ts`, and `zombie-503-reason-field.test.ts`: beforeAll spawn a relay subprocess on an ephemeral port with the PRODUCTION_PORTS guard, kill in afterAll. ~30 tests to migrate; ~1h work. Not blocking; gated on Priority #1 comms work completing first.
- [ ] **[relay/security — PARKED per CEO 2026-04-16]** Local-process spoofing possible. Any process running as the CEO's user on the Mac can POST /send as any agent: it can (a) write a port file to `~/.claude/relay-channel/{name}.port` to pass the sender allowlist, (b) read `RELAY_SECRET` from a legitimate agent's environment if activated, and (c) reach port 8767 directly since the relay binds 0.0.0.0. Parked per CEO ruling: tailnet is CEO-devices-only, no hostile peer model applies, so the sender allowlist + default-no-secret posture is accepted. **Un-park conditions:** (1) tailnet is ever shared with a non-CEO device or user, (2) a Windows or remote-host port of the relay, (3) any production/multi-tenant deploy, (4) CEO runs untrusted local code (downloaded installers, third-party extensions with FS access, shared-user accounts). Revisit the 7-level ladder in `proposals/relay-identity-auth-2026-04-16T00-46-56.md` when any trigger fires.
