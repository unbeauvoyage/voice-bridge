namespace ContentService.Features.OpenApi;

/// <summary>
/// GET /openapi.yaml — serves the byte-identical TS OpenAPI document
/// that ships with content-service/docs/. The file is brought into the
/// build output via a Linked &lt;Content&gt; item in ContentService.csproj
...
internal static class OpenApiEndpoint
{
    ...
    private static async Task HandleAsync(HttpContext httpContext, CancellationToken cancellationToken)
    {
        ...
        {
            await RespondMissingAsync(httpContext, cancellationToken);
        }
    ...
    private static async Task RespondMissingAsync(HttpContext httpContext, CancellationToken cancellationToken)
    {
        ...
        await httpContext.Response.WriteAsync(
            "openapi.yaml is missing from the build output. Expected the Linked <Content> item to copy ../content-service/docs/openapi.yaml.",
            cancellationToken);
    }
}
