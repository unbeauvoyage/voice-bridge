...
export async function llmRoute(
  ...
  try {
    ...
    const agentNorm = typeof llmParsed.agent === 'string'
    ...
    const agent = knownNorm.length === 0 || knownNorm.includes(agentNorm) ? agentNorm : fallbackAgent;
    const agentChanged = agent !== fallbackAgent;

    console.log(`[llmRoute] fragment="${fragmentToMatch}" → agent="${agent}" (changed=${agentChanged})`);
