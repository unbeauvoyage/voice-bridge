...
export async function deliverToAgent(transcript: string, to: string): Promise<void> {
  ...
  const relayObj = await res.json() as SendResponse;
  if (relayObj.status === 'queued') {
  ...
  fetch(RELAY_URL, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ from: "ceo", to: "ceo", type: "voice-sent", body: transcript, meta: { sentTo: to } }),
    signal: AbortSignal.timeout(RELAY_TIMEOUT_MS),
  }).catch(() => {});
}
