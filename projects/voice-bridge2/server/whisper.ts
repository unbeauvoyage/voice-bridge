...
function convertToWav(audioBuffer: Buffer, ext: string): Buffer {
  ...
  try {
    ...
    const ffOut = execSync(`ffmpeg -i "${inputPath}" -ar 16000 -ac 1 -c:a pcm_s16le "${wavPath}" -y 2>&1`, { timeout: 60000 });
    console.log(`[whisper] ffmpeg: ${ffOut.toString().split('\n').filter(l => l.includes('Duration') || l.includes('Output')).join(' | ')}`);
    const wav = readFileSync(wavPath);
    const wavSamples = (wav.length - 44) / 2; // 16-bit samples
    console.log(`[whisper] wav: ${wav.length} bytes, ${wavSamples} samples, ${(wavSamples / 16000).toFixed(1)}s`);
    return wav;
  } finally {
    try { unlinkSync(inputPath); } catch { /* tmp file may already be gone */ }
    try { unlinkSync(wavPath); } catch { /* tmp file may already be gone */ }
  }
}
...
export async function transcribeAudio(audioBuffer: Buffer, mimeType: string): Promise<string> {
  ...
  const wavBuffer = process.env.WHISPER_SKIP_CONVERT === "1"
    ? audioBuffer
    : convertToWav(audioBuffer, ext);
