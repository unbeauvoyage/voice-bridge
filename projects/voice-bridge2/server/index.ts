  ...
  async fetch(req) {
    ...
    if (req.method === 'POST' && url.pathname === '/transcribe') {
        ...
        if ('inProgress' in existing) {
          ...
          while (Date.now() < deadline) {
            ...
            if (!updated || !('inProgress' in updated)) {
              if (updated) {
                console.log(
                  `[voice-bridge] duplicate resolved after wait, returning cached transcript`
                )
                return Response.json(
      ...
      if (micCmd) {
        ...
        return Response.json(
          { transcript, mic: micCmd.state, command: true },
          { headers: corsHeaders }
        )
      }
      ...
      if (pleaseInFirst7) {
        ...
        console.log(
          `[route] → ${to} (please-gate, changed=${llmResult.agentChanged}): "${message}"`
        )
      } else if (explicitTo) {
